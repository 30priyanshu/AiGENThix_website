...
// Simple formatter to handle bold (**text**) and bullet points (* point)
const formatMessage = (content) => {
  ...
  lines.forEach((line, index) => {
    ...
    if (trimmedLine.startsWith('* ') || trimmedLine.startsWith('- ')) {
      ...
      const formattedBullet = formatBold(bulletContent);
      ...
      if (trimmedLine === '') {
        ...
        formattedLines.push(<p key={`p-${index}`} className="mb-2 last:mb-0">{formatBold(trimmedLine)}</p>);
      }
    }
  });
